...
export function calculateSMSSSV(
  ...
  if (move.named('Photon Geyser', 'Light That Burns the Sky', 'Corrupted Humanity') ||
  ...
  const defenderAbilityIgnored = defender.hasAbility(
    'Abyss Treader', 'Armor Tail', 'Aroma Veil', 'Aura Break', 'Battle Armor',
    'Big Pecks', 'Bulletproof', 'Clear Body', 'Contrary',
    'Damp', 'Dazzling', 'Disguise', 'Dragontamer', 'Dry Skin',
    'Earth Eater', 'Filter', 'Flash Fire', 'Flower Gift',
    'Flower Veil', 'Fluffy', 'Friend Guard', 'Frontal Guard', 'Fur Coat',
    'Good as Gold', 'Grass Pelt', 'Guard Dog', 'Heatproof',
    'Heavy Metal', 'Hyper Cutter', 'Ice Face', 'Ice Scales',
    'Illuminate', 'Immunity', 'Inner Focus', 'Insomnia', 'Izalith',
    'Keen Eye', 'Kingly Majesty', 'Leaf Guard', 'Levitate', 'Light Metal',
    'Lightning Rod', 'Limber', 'Lion Stance', 'Magic Bounce', 'Magma Armor',
    ...
    'Sticky Hold', 'Storm Drain', 'Stormwalker', 'Sturdy', 'Suction Cups',
    'Sweet Veil', 'Tangled Feet', 'Telepathy', 'Tera Shell',
    'Thermal Exchange', 'Thick Fat', 'Toxic Ailment', 'Unaware', 'Vital Spirit',
    'Volt Absorb', 'Water Absorb', 'Water Bubble', 'Water Veil',
    'Weakspot', 'Well-Baked Body', 'White Smoke', 'Wind Rider',
    'Wonder Fat', 'Wonder Guard', 'Wonder Skin'
  );
  ...
  const ignoresNeutralizingGas = [
    'As One (Glastrier)', 'As One (Ruin)', 'As One (Spectrier)', 'Battle Bond', 'Comatose',
  ...
  if (move.originalName === 'Weather Ball') {
    ...
    type =
      ...
      : field.hasTerrain('Darkness') ? 'Dark'
      : field.hasTerrain('Poison') ? 'Poison'
      : field.hasTerrain('Light') ? 'Normal'
      : 'Normal';
  ...
  if (attacker.hasAbility('Overwhelming Heat') && move.hasType('Fire') && defender.hasType('Fire')){
    typeEffectiveness = 2;
    desc.attackerAbility = attacker.ability;
  }
  if (attacker.hasAbility('Superior') && move.hasType(...defender.types)){
    typeEffectiveness = 2;
    desc.attackerAbility = attacker.ability;
  }
  ...
  if ((defender.hasAbility('Wonder Guard') && typeEffectiveness <= 1) ||
      ...
      (move.hasType('Fire') && defender.hasAbility('Flash Fire', 'Well-Baked Body', 'Izalith')) ||
      ...
      (move.hasType('Poison') && defender.hasAbility('Toxic Ailment')) ||
      (move.flags.bullet && defender.hasAbility('Bulletproof')) ||
      (move.flags.sound && !move.named('Clangorous Soul') && defender.hasAbility('Soundproof')) ||
      (move.priority > 0 && defender.hasAbility('Queenly Majesty', 'Dazzling', 'Armor Tail', 'Kingly Majesty')) ||
      (move.hasType('Ground') && defender.hasAbility('Earth Eater')) ||
      (move.flags.wind && defender.hasAbility('Wind Rider')) ||
      (defender.hasAbility('Frontal Guard') && defender.abilityOn  &&  hitsPhysical) ||
      (move.hasType('Dragon', 'Flying') && defender.hasAbility('Dragontamer')) ||
      (defender.hasAbility('Invulnerable'))
  ) {
    desc.defenderAbility = defender.ability;
    return result;
  }
...
export function calculateBasePowerSMSSSV(
  ...
  case 'Rising Voltage':
    ...
    break;
  case 'Wind Blade':
    basePower = move.bp * (field.hasWeather('Strong Winds') ? 1.5 : 1);
  ...
  case 'Psyblade':
    ...
    break;
  case 'Corrupted Humanity':
    basePower = move.bp * (field.hasTerrain('Darkness') ? 1.5 : 1);
    if (field.hasTerrain('Darkness')){
      desc.moveBP = basePower;
      desc.terrain = field.terrain;
    }
    break;
...
export function calculateBPModsSMSSSV(
  ...
  if (isGrounded(attacker, field)) {
    if ((field.hasTerrain('Electric') && move.hasType('Electric')) ||
        ...
        (field.hasTerrain('Light')  && move.hasType('Normal'))
    ) {
  ...
  if (isGrounded(defender, field)) {
    if ((field.hasTerrain('Misty') && move.hasType('Dragon')) ||
        ...
        (field.hasTerrain('Light') && move.hasType('Dark'))
    ) {
      bpMods.push(2048);
      desc.terrain = field.terrain;
    }
  ...
  if (attacker.hasItem(`${move.type} Gem`)) {
    ...
    bpMods.push(4505);
    desc.attackerItem = attacker.item;
  } else if(attacker.hasAbility('Dragonslayer') && move.hasType('Steel', 'Electric') && defender.types.includes('Dragon')){
    bpMods.push(8192);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Traitor') && attacker.abilityOn){
    bpMods.push(5320);
    move.breaksProtect;
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Blindside') && field.defenderSide.isProtected){
    !field.defenderSide.isProtected;
    bpMods.push(2048);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Stormwalker') && field.hasWeather('Strong Winds') && move.hasType('Flying')){
    bpMods.push(6144);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Expierienced') && move.hasType(...attacker.types)){
    bpMods.push(4092);
    desc.attackerAbility = attacker.ability;
  }
    else if (attacker.hasAbility('Expierienced')){
    bpMods.push(6144);
    desc.attackerAbility = attacker.ability;
  }
...
export function calculateAtModsSMSSSV(
  ...
  if ((attacker.hasAbility('Slow Start') && attacker.abilityOn &&
  ...
  } else if (attacker.hasAbility('Izalith') && attacker.abilityOn && move.hasType('Dark')){
    atMods.push(6144);
    desc.attackerAbility = 'Izalith';
  } else if (
    (attacker.hasAbility('Steelworker') && move.hasType('Steel')) ||
    (attacker.hasAbility('Dragon\'s Maw') && move.hasType('Dragon')) ||
    (attacker.hasAbility('Rocky Payload') && move.hasType('Rock')) ||
    (attacker.hasAbility('Abyss Treader') && move.hasType('Dark')) ||
    (attacker.hasAbility('Gwynchild') && move.hasType('Electric'))
  ) {
    atMods.push(6144);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Demonborne') && (move.hasType('Fire', 'Dark'))){
    atMods.push(6144);
    ...
    (attacker.hasAbility('Hearthbearer') && move.hasType('Fire')) ||
    (attacker.hasAbility('Huge Power', 'Pure Power') && move.category === 'Physical') ||
    (attacker.hasAbility('Draconic Might') && attacker.abilityOn)
  ) {
    atMods.push(8192);
    desc.attackerAbility = attacker.ability;
  }
  ...
  if ((defender.hasAbility('Thick Fat') && move.hasType('Fire', 'Ice')) ||
      ...
      (defender.hasAbility('Abyss Treader') && move.hasType('Dark'))) {
    atMods.push(2048);
    desc.defenderAbility = defender.ability;
  }
  ...
  if ((attacker.hasItem('Thick Club') &&
    ...
    atMods.push(6144);
    desc.attackerItem = attacker.item;
  } else if (attacker.hasAbility('Bloodlust')){
    atMods.push(6144);
    desc.attackerAbility = attacker.ability;
  } else if (attacker.hasAbility('Double Pain')){
    atMods.push(5461);
    desc.attackerAbility = attacker.ability;
  }
  return atMods;
...
export function calculateFinalModsSMSSSV(
  ...
  if (defender.hasAbility('Weakspot')) {
  ...
  if (defender.hasAbility('Dark Shroud') && field.hasTerrain('Darkness')){
    finalMods.push(2048);
    desc.defenderAbility = defender.ability;
  }
  ...
  if (defender.hasAbility('Fluffy', 'Toxic Ailment') && move.hasType('Fire') ||
      defender.hasAbility('Automaton') && move.hasType('Psychic') ||
      defender.hasAbility('Spiritual Projection') && move.hasType('Ghost')) {
    finalMods.push(8192);
    desc.defenderAbility = defender.ability;
  }
  ...
  if (attacker.hasItem('Expert Belt') && typeEffectiveness > 1 && !move.isZ) {
    ...
    finalMods.push(5324);
    desc.attackerItem = attacker.item;
  } else if (attacker.hasItem('Metronome') || attacker.hasAbility('Proficiency') && move.timesUsedWithMetronome! >= 1) {
